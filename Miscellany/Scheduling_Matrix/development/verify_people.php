#! /usr/local/bin/php
<?php
//  Add people to the db who share a common email address and got ommitted originally.
//  (buncha sociologists)
...
function process($contact)
{
  ...
  {
    if ( ($contact->last_name !== $contacts[$key]->last_name) && ($contact->email === 'strickmcaul@msn.com') )
    {
      $result = pg_query($db, "INSERT INTO people VALUES "
          . "( default, '{$contact->last_name}', '{$contact->first_name}', '{$contact->honorific}', "
          . " '{$contact->affiliation}', '{$contact->email}', default)");
      if ($result) echo "Added {$contact->last_name}\n";
      else echo "*** Failed to add {$contact->last_name}\n";
    }
